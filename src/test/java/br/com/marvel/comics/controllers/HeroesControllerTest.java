...
class HeroesControllerTest {
    ...
    @Autowired
    private JacksonTester<CharacterDataWrapper> characterDataWrapperJson;

    @Test
    void listComics() throws Exception {
        CharacterDataWrapper expected = new CharacterDataWrapper();
        when(service.listHeroes()).thenReturn(expected);
        ...
        String expectedJson = characterDataWrapperJson.write(expected).getJson();
